...
class PolynomialRegression() :
    ...
    def generate_polynomial_features(self, X) :
        ...
        for i in range(2, m + 1):
            ...
            for j in range(len(X[:, 1])):
                col.append(X[j][1] ** i)
            col = np.array(col)
            X = np.concatenate((X, col[:, None]), axis=1)
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            self.coef_ = self.coef_ - eta * 2 * np.dot(X.T, np.dot(X, self.coef_) - y)
    ...
    def rms_error(self, X, y) :
        ...
        error = math.sqrt(self.cost(X, y) / float(n))
        ### ========== TODO : END ========== ###
        return error
...
def main() :
    ...
    step_sizes = [0.01, 0.001, 0.0001, 0.0407]
    for s in step_sizes:
        model.coef_ = np.zeros(2)
        print("Step size {}".format(s))
        start_time = time.time()
        model.fit_GD(train_data.X, train_data.y, eta=s)
        end_time = time.time()
    ...
    print 'Gradient Descent with Step Size Dependent on Iteration:'
    model.coef_ = np.zeros(2)
    start_time = time.time()
    ...
    for i in range(0, 11):
        ...
        model.fit(train_data.X, train_data.y)
        curr_train_err = model.rms_error(train_data.X, train_data.y)
        train_errors.append(curr_train_err)
        curr_test_err = model.rms_error(test_data.X, test_data.y)
    ...
    plt.title("Errors of Polynomial Regression of Varying Degrees")
